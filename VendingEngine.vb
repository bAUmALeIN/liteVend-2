...
#Const BASKET_MAX = 30
    ...
    Public Function processEingabe(panelCtrls As Control.ControlCollection, InputBox As TextBox, OutputBox As RichTextBox, flp As FlowLayoutPanel, flpNum As FlowLayoutPanel) As Boolean
        If My.Settings.isFirstOrder OrElse (My.Settings.MultiVend AndAlso Not My.Settings.needMenge) Then
            ...
            Console.WriteLine(".ProccesEingabe: ID Trimmed: " & ID)
            ...
            For Each ctrl In panelCtrls
                If TypeOf ctrl Is Button AndAlso ctrl.Name = "btnItem" & ID Then
                    ...
                    Console.WriteLine("VendingEngine.ProccesEingabe: ermittelter Preis: " & preis)
                    ...
                    Console.WriteLine("VendingEngine.ProccesEingabe: Anzahl Basket:" & Basket.Count)
                    My.Settings.ActiveMengeID = Convert.ToInt32(ID)
                    Console.WriteLine("VendingEngine.ProccesEingabe: ActiveMengenID gesetzt: " & My.Settings.ActiveMengeID)
                    Console.WriteLine("VendingEngine.ProccesEingabe: OrderNum BasketEintrag zugewiesen: " & My.Settings.OrderNum)
            ...
            If Not produktGefunden Then
                ...
                Console.WriteLine("VendingEngine.processEingabe: Produkt nicht gefunden")
            End If
        Else
            ' Mengenvorgang
            If InputBox.TextLength = 0 Then
                Console.WriteLine("VendingEngine.processEingabe: InputBox Leer")
                Return False
            End If
            ...
            If Not Integer.TryParse(Menge, intMenge) Then
                Console.WriteLine("VendingEngine.processEingabe: Ungültige Menge")
                Return False
            End If
            ...
            gesamtpreis = 0
            InputBox.Text = ""
            Console.WriteLine("VendingEngine.processEingabe -> Mengenvorgang.geleseneMenge: " & Menge)

            For i As Integer = 0 To Basket.Count - 1
                If Basket(i).OrderNum = My.Settings.OrderNum - 1 Then
                    Console.WriteLine("VendingEngine.processEingabe -> Mengenvorgang.SucheOrderNum: " & Basket(i).OrderNum)
                    ...
                    Console.WriteLine("VendingEngine.processEingabe -> Mengenvorgang.gefundenOrderNum: " & Basket(i).OrderNum & " | gesPreis: " & Basket(i).gesPreis)
                    ID = Basket(i).ID
            ...
            If My.Settings.MultiVend Then
                Console.WriteLine("VendingEngine.processEingabe: MULTIVEND 1")
            ...
            Console.WriteLine("VendingEngine.processEingabe: MULTIVEND 0")
            Console.WriteLine("VendingEngine.processEingabe: Start Zahlung.... WIP")
    ...
    Public Function ProcessZahlung(selectedAmount As Double, OutputBox As RichTextBox) As Boolean
        ...
        If restbetrag <= 0 Then
            ...
            Console.WriteLine($"alter GesamtUmsatz = {stats.GesUmsatz}")
            Console.WriteLine($"{My.Settings.BasketGesPreis.ToString}")
